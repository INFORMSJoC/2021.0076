...
#            l2 = tf.nn.dropout(l2,0.8)
    ...
    def predict(self,input_data):
        ...
        return_label[np.where(return_label==-1)] = 0
        return return_label
...
def mlp_vae_predict(train,test,test_label):
    ...
    mlp_vae.train(train)
    mlp_vae_predict_label = mlp_vae.predict(test) 
...
def iforest_predict(train,test,test_label):
    ...
    iforest = IsolationForest(max_samples = 'auto', contamination=0.01)
...
if __name__ == '__main__':
    ...
    y_test[np.where(y_test==-1)] = 0 #把原来的多数类用0表示，个人习惯

    VAE_clf = MLP_VAE(8,20,0.07)
    VAE_clf.train(X_train)
    y_pred = VAE_clf.predict(X_test) 
